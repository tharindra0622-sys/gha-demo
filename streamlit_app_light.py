"""
CI/CD Failure Prediction — Live Dashboard (light presentation edition)
...
Deploy:        Streamlit Cloud, or your AWS EC2 instance
"""
...
# ---------------------------------------------------------------------
# Visual polish: fonts, metric cards, status badges — light theme
# ---------------------------------------------------------------------
st.markdown("""
<style>
...
/* Hero title block */
.hero-title { font-size: 2.1rem; font-weight: 700; color: #111827; margin-bottom: 0.1rem; }
.hero-sub   { color: #6B7280; font-size: 0.95rem; margin-bottom: 1.4rem; }
...
.stTabs [data-baseweb="tab"] {
    background-color: #F9FAFB; border-radius: 8px 8px 0 0;
    padding: 8px 16px; color: #6B7280; font-weight: 500;
}
...
/* Expander (AIOps diagnosis cards) */
...
st.markdown(
    ...
    f'source: <a href="https://github.com/{GITHUB_USER}/{GITHUB_REPO}" style="color:#2563EB;">'
...
with tab1:
    ...
    try:
        ...
        st.dataframe(cv_df.style.highlight_max(axis=0, color="#DBEAFE"), use_container_width=True)
...
with tab3:
    ...
    try:
        ...
        st.bar_chart(chart_df.set_index("run_number"), color="#2563EB")
    except Exception as e:
...
with tab5:
    ...
    control_agent.py  → finds failed / predicted-failure runs
            ↓
    langchain_multi_tool_agent_v2.py  → Claude (via LangChain) checks
        up to 7 tools — failure log, past issues, workflow file, GitHub
        status, the open web, package registry status, recent commits —
        before diagnosing root cause + suggesting a fix
            ↓
